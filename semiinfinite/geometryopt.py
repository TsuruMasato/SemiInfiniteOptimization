...
class RobotTrajectoryCollisionConstraint(SemiInfiniteConstraintInterface):
    ...
    def minvalue(self,x,bound=None):
        ...
        for i in range(len(traj.milestones)-1):
            ...
            if len(active) > 0:
                if self.verbose>=2: 
                    ...
                    for (link,env),d in active.items():
                        if d < alinks[link]:
                            alinks[link] = d
                        if d < aenvs[env]:
                            aenvs[env] = d
        ...
        while len(edgequeue) > 0:
            ...
            assert 0 < midpt < 1
            duration = (v - u)
            q = traj.interpolate_state(traj.milestones[edge],traj.milestones[edge+1],midpt,traj.times[edge+1]-traj.times[edge])
